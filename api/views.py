...
# Cache compartido para el streaming
_stream_cache = {
    'data': None,
    'time': None,
...
def crypto_stream(request, num_coins=None):
    def event_stream():
        service = CryptoService()
        while True:
            now = datetime.now()
            if (_stream_cache['data'] is None or _stream_cache['time'] is None or 
                now - _stream_cache['time'] > _stream_cache['duration']):
                try:
                    if num_coins:
                    ...
                    _stream_cache['data'] = json.dumps(data_to_cache)
                    _stream_cache['time'] = now
                    
                    yield f"data: {_stream_cache['data']}\n\n"
